package space.arkady.mvvmclonespotify.data.ui
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        navHostFragment.findNavController().addOnDestinationChangedListener { _, destination, arguments ->
            when (destination.id) {
    ...
    private fun subscribeToObservers() {
                ...
                when (result.status) {
                    SUCCESS -> {
                        result.data?.let { songs ->
                            ...
                            if (songs.isNotEmpty()) {
                                glide.load(curPlayingSong ?: songs[0].imageUri).into(ivCurSongImage)
                            }
        ...
        mainViewModel.curPlayingSong.observe(this) {
            ...
            glide.load(curPlayingSong?.imageUri).into(ivCurSongImage)
            switchViewPagerToCurrentSong(curPlayingSong ?: return@observe)
        }
        mainViewModel.playbackState.observe(this) {
            ...
            ivPlayPause.setImageResource(
                if (playbackState?.isPlaying == true)
                    R.drawable.ic_baseline_pause_24 else R.drawable.ic_baseline_play_arrow_24
            )
        }
